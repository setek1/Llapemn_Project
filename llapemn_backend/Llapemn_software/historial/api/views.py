...
class HistorialApiViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=['GET'])
    def get_upperdata(self, request):
         queryset = Insumo.objects.all().annotate(cantidad_total_insumos=Count('id'))
         cantidad_total_insumos = queryset.count()

         queryset_salas=Salas.objects.all().annotate(cantidad_total_salas=Count('id'))
         cantidad_total_salas=queryset_salas.count()

         queryset_users = User.objects.all().annotate(total_users=Count('id'))
         cantidad_total_user = queryset_users.count()

         total_insumos_bajos=Insumo.objects.filter(stockIn__lt=5).count()
         return Response({
             'cantidad_total_insumos': cantidad_total_insumos,
             'cantidad_total_salas':cantidad_total_salas,
             'cantidad_total_users':cantidad_total_user,
             'insumos_bajos':total_insumos_bajos,
             })