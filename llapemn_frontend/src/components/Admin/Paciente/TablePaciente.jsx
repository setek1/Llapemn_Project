...
export function TablePaciente(props) {
  const { paciente, updatePaciente, deletePaciente } = props;
  return (
      ...
      <table className="mt-5  w-full ">
        ...
        <tbody className="text-center dark:text-white">
          {map(paciente, (paci, index) => (
            <tr key={index}>
              <td className="pb-6 pr-6 pt-6 text-left">{paci.nombre}</td>
...
function Actions(props) {
  const { pacientes, deletePaciente, updatePaciente } = props;
  return (
    <>
      <td>
        <button onClick={() => updatePaciente(pacientes)}>
          <IoBuildOutline />
        </button>
      </td>
      <td>
        <button onClick={() => deletePaciente(pacientes)}>
          <IoTrashBinOutline />
        </button>
      </td>
    </>
